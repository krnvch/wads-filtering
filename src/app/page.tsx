...
// Generate deterministic "last seen" dates relative to now for realistic filtering
function hoursAgo(h: number): string {
  return new Date(Date.now() - h * 3600_000).toISOString();
}

const MOCK_ATTACKS: Attack[] = [
  { id: "atk_01", name: "XSS in query.filter", type: "XSS", status: "Blocked", impact: "High", response_code: 200, host: "api.example.com", endpoints: "GET /v1/api/search", parameter: "query.filter", blocking_status: "Active blocking", sources: { countries: ["US", "UK"], ips: ["10.0.0.1"] }, timeline: { last_seen: hoursAgo(2), first_detected: hoursAgo(480) } },
  { id: "atk_02", name: "SQL Injection in login", type: "SQL Injection", status: "Monitored", impact: "High", response_code: 401, host: "auth.example.com", endpoints: "POST /v1/auth/login", parameter: "body.username", blocking_status: "Passive monitoring", sources: { countries: ["Italy"], ips: ["10.0.0.2"] }, timeline: { last_seen: hoursAgo(6), first_detected: hoursAgo(1200) } },
  { id: "atk_03", name: "BOLA on orders endpoint", type: "BOLA Attack", status: "Blocked", impact: "High", response_code: 403, host: "orders.example.com", endpoints: "GET /v1/api/orders/{id}", parameter: "path.id", blocking_status: "Active blocking", sources: { countries: ["China"], ips: ["10.0.0.3"] }, timeline: { last_seen: hoursAgo(18), first_detected: hoursAgo(360) } },
  { id: "atk_04", name: "Scraper bot on catalog", type: "Scraper Bot", status: "Started", impact: "Low", response_code: 200, host: "shop.example.com", endpoints: "GET /v1/catalog", parameter: "query.page", blocking_status: "Not configured", sources: { countries: ["Russia"], ips: ["10.0.0.4"] }, timeline: { last_seen: hoursAgo(30), first_detected: hoursAgo(72) } },
  { id: "atk_05", name: "Brute force login attempts", type: "Brute Force", status: "Monitored", impact: "Medium", response_code: 401, host: "auth.example.com", endpoints: "POST /v1/auth/login", parameter: "body.password", blocking_status: "Passive monitoring", sources: { countries: ["Brazil"], ips: ["10.0.0.5"] }, timeline: { last_seen: hoursAgo(48), first_detected: hoursAgo(2160) } },
